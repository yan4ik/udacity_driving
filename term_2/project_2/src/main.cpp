...
void output_step(const Fusion_UKF& fusion_ukf,
                 const Data_Package& measurement,
                 const Data_Package& ground_truth,
                 ofstream* out_file) {

    *out_file << fusion_ukf.state_(0) << "\t";
    *out_file << fusion_ukf.state_(1) << "\t";
    *out_file << fusion_ukf.state_(2) << "\t";
    *out_file << fusion_ukf.state_(3) << "\t";
    *out_file << fusion_ukf.state_(4) << "\t";
    ...
    *out_file << ground_truth.data_(3) << "\t";
    ...
    if (measurement.sensor_type_ == Data_Package::LASER) {
        ...
        *out_file << fusion_ukf.NIS_laser_ << endl;
    
    } else if (measurement.sensor_type_ == Data_Package::RADAR) {
          
        *out_file << fusion_ukf.NIS_radar_ << endl;
...
int main(int argc, char* argv[]) {
    ...
    for (size_t i = 0; i < N; ++i ) {
        ...
        fusion_ukf.process_measurement(measurement_pack_list[i]);

        output_step(fusion_ukf, 
                    measurement_pack_list[i], 
                    gt_pack_list[i],
                    &out_file_);


        Vector4d ukf_state_cartesian = Vector4d();
        ukf_state_cartesian << fusion_ukf.state_(0),
                               fusion_ukf.state_(1),
