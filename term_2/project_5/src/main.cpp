...
void global2vehicle(double px, double py, double psi,
                    vector<double>* ptsx,
                    vector<double>* ptsy) {

    for (int i = 0; i < ptsx->size(); ++i) {
        double diff_x = ptsx->at(i) - px;
        double diff_y = ptsy->at(i) - py;

        ptsx->at(i) = diff_x * cos(psi) + diff_y * sin(psi);
        ptsy->at(i) = diff_y * cos(psi) - diff_x * sin(psi);
    }
      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          */
          ...
          // battling lattency - looking into the future
          double latency = 0.1;
